...
function loadPatientList() {
  fetch("/patients")
    ...
    .then((patients) => {
      ...
      if (isAddingPatient) {
        listContent +=
          '<li><input type="text" class="add-patient-input" id="new-patient-input" placeholder="Enter patient name"></li>';
      }
      listContent += patients
        .map(
          (patient) =>
...
function saveNote() {
  ...
  const editTextarea = document.getElementById("edit-response");
  const content =
    editTextarea.style.display === "block" ? editTextarea.value : noteDisplay.textContent;
  console.log(content);
